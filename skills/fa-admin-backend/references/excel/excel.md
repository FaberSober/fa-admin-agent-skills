...
简单读取可使用 `FaExcelUtils.simpleRead(...)`，在回调中完成行级基础校验和转换。复杂导入同时遵循 [../data/mapper-biz.md](../data/mapper-biz.md) 的批量 Upsert 规则：一次查询已有数据、构建 Map、拆分新增/更新集合、按批写入。
...
批量写入慢的排查（JDBC `rewriteBatchedStatements`、批次大小、实际 SQL）见 [../data/mapper-biz.md](../data/mapper-biz.md) 的批量写入节。
